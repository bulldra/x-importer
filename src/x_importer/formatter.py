...
def _format_quoted(
    ref_tweet: dict, ref_map: dict[str, dict], depth: int = 1
) -> list[str]:
    prefix = "> " * depth
    empty_prefix = prefix.rstrip()
    author = ref_tweet.get("_author_username", "")
    quoted_text = _expand_urls(ref_tweet["text"], ref_tweet.get("entities", {}))
    lines: list[str] = []
    if author:
        lines.append(f"{prefix}@{author}:")
        lines.append(empty_prefix)
    lines.append(prefix + quoted_text.replace("\n", f"\n{prefix}"))
    ...
    if depth < _MAX_QUOTE_DEPTH:
        for ref in ref_tweet.get("referenced_tweets", []):
            if ref["type"] in ("quoted", "retweeted", "replied_to") and ref["id"] in ref_map:
                lines.append(empty_prefix)
                lines.extend(_format_quoted(ref_map[ref["id"]], ref_map, depth + 1))
...
def _format_tweet_body(
    tweet: dict, ref_map: dict[str, dict], skip_replied_ids: set[str] | None = None,
) -> list[str]:
    """ツイート本文・引用RTを出力（ヘッダ・メトリクス除く）"""
    lines: list[str] = []
    ...
    for ref in tweet.get("referenced_tweets", []):
        if ref["type"] == "replied_to" and ref["id"] in ref_map and ref["id"] not in skip:
            lines.extend(_format_quoted(ref_map[ref["id"]], ref_map))
            lines.append("")
...
def format_tweet(tweet: dict, username: str, ref_map: dict[str, dict]) -> str:
    dt_jst = _parse_dt(tweet["created_at"])
    ...
    if is_rt:
        ...
        lines.extend(_format_tweet_body(tweet, ref_map))
...
def format_thread(
    chain: list[dict], username: str, ref_map: dict[str, dict]
) -> str:
    ...
    for tweet in chain:
        lines.extend(_format_tweet_body(tweet, ref_map, skip_replied_ids=chain_ids))
...
def _format_analytics(tweets: list[dict]) -> str:
    ...
    lines.append("")
    lines.append("| Posts | Like | RT | Reply | Imp |")
    lines.append("|------:|-----:|---:|------:|----:|")
    lines.append(
        f"| {len(own_tweets)} | {total_like} | {total_rt} | {total_reply} | {total_imp} |"
    )
    return "\n".join(lines)


def format_day(
    date_str: str, tweets: list[dict], username: str, ref_map: dict[str, dict]
) -> str:
    ...
    for tweet in tweets:
        ...
        if tweet["id"] in chain_heads:
            lines.append(format_thread(chain_heads[tweet["id"]], username, ref_map))
        else:
            lines.append(format_tweet(tweet, username, ref_map))
        lines.append("")
...
def write_markdown_files(
    ...
    for date_str in sorted(groups.keys()):
        content = format_day(date_str, groups[date_str], username, ref_map)
